...
export async function renovarRecorrencias() {
  ...
  try {
    ...
    for (const recorrencia of recorrenciasAtivas) {
      const ultimaDataGerada =
        recorrencia.transacoes.length > 0
          ? recorrencia.transacoes[0].dataVencimento
          : new Date(recorrencia.dataInicio.getTime() - 1000)

      const limiteIndividual =
        recorrencia.dataFim && recorrencia.dataFim < horizonteLimite
          ? recorrencia.dataFim
          : horizonteLimite
      ...
      const datasNovas = todasDatas.filter(
        (d) => d.getTime() > ultimaDataGerada.getTime(),
      )
      ...
      if (datasNovas.length > 0) {
        const transacoesAGerar = datasNovas.map((data) => ({
          ...
          subcategoriaId: recorrencia.subcategoriaId,
    ...
    console.log(
      `✅ [Cron] ${totalCriadas} transações futuras geradas para manter o horizonte de 12 meses.`,
    )
  } catch (error) {
